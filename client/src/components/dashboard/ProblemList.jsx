const ProblemList = ({ problems, cardStyle = {} }) => {
  ...
  return (
    <div className="overflow-x-auto -mx-4 sm:mx-0">
      {problems.length === 0 ? (
        <div className="bg-[#dbeafe] rounded-xl p-4 sm:p-6 text-center mx-4 sm:mx-0">
          <div className="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-[#e2e8f0] mb-3 sm:mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 sm:h-8 sm:w-8 text-[#94a3b8]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
          </div>
          <p className="text-[#333333] font-medium text-sm sm:text-base">No problems found in this category.</p>
          <p className="text-[#555555] text-xs sm:text-sm mt-1">Try adding some problems or changing your filter.</p>
        </div>
      ) : (
        <div className="bg-[#dbeafe] rounded-xl overflow-hidden shadow-sm mx-4 sm:mx-0">
          <div className="block sm:hidden">
            {/* Mobile view - card layout */}
            {problems.map((problem) => (
              <div 
                key={problem.id} 
                className="p-4 border-b border-blue-100 last:border-b-0"
              >
                <div className="flex justify-between items-start mb-2">
                  <a 
                    ...
                    className="text-sm font-medium text-[#333333] hover:text-blue-600 hover:underline transition-colors pr-2"
                  >
                    {problem.title}
                  </a>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getPlatformBadgeColor(problem.platform)}`}>
                    {problem.platform}
                  </span>
                  <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${getStatusColor(problem.status)}`}>
                    {problem.status === 'solveLater' ? 'Solve Later' : problem.status.charAt(0).toUpperCase() + problem.status.slice(1)}
                  </span>
                </div>
                <div className="text-xs text-[#555555]">
                  Added on: {problem.dateAdded}
                </div>
              </div>
            ))}
          </div>
